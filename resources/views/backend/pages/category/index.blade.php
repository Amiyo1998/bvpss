        ...
        <div class="row">
            <div class="col-md-8 mx-auto py-5">
                ...
                <div class="bg-primary justify-content-between d-flex px-1 py-1 mb-3 border rounded-3">
                    <div class="mt-3">
                        <p class="bg-white p-1 border rounded-3 fs-4 fw-bold">{{ __("All category")}}</p>
                    </div>
                    <div class="mt-3">
                        <a href="{{ route('categories.create')}}">
                            <p  class="text-dark bg-white p-1 border rounded-3 fs-4 fw-bold">{{ __("Add Category")}}</p>
                        </a>
                    </div>
                </div>
                <table class="table table-striped table-hover  border-2">
                    <tr class=" border-2">
                        ...
                        <th class="border-2">{{ __("Category name")}}</th>
                    ...
                    @if ($categories->count())
                        @foreach ($categories as $key=>$cat)
                            <tr>
                                ...
                                <td class="border-2">{{ $cat->name}}</td>
                                <td class="border-2 ">
                                    <a href="{{ route('categories.edit', $cat->id)}}" class="text-light bg-primary d-inline-block p-1 border rounded-3 fs-4 fw-bold">{{__("Edit")}}</a>
